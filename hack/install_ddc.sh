...
# Modified by pngouin to macth defectdojo-cli


# If no argument is given -> Downloads the most recently released
# defectdojo-cli binary to your current working directory.
# (e.g. 'install_ddc.sh')
#
# If one argument is given -> 
# If that argument is in the format of #.#.#, downloads the specified
# version of the DefectDojo-CLI binary to your current working directory.
# If that argument is something else, downloads the most recently released
# DefectDojo-CLI binary to the specified directory.
# (e.g. 'install_ddc.sh 3.8.2' or 'install_ddc.sh $(go env GOPATH)/bin')
#
# If two arguments are given -> Downloads the specified version of the
# DefectDojo-CLI binary to the specified directory.
# (e.g. 'install_ddc.sh 3.8.2 $(go env GOPATH)/bin')
#
...
release_url=https://api.github.com/repos/pngouin/defectdojo-cli/releases
if [ -n "$1" ]; then
  if [[ "$1" =~ ^[0-9]+(\.[0-9]+){2}$ ]]; then
    version=v$1
